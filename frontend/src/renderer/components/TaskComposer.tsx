...
export function TaskComposer({
	...
	const refreshModels = useMutation({
		mutationFn: () => refreshAgentModels(selectedAgent, projectId ?? ""),
		onSuccess: (catalog) =>
			queryClient.setQueryData(agentModelsQueryKey(selectedAgent, projectId ?? ""), catalog),
	});
	const modelWarning =
		(refreshModels.isError
			? refreshModels.error instanceof Error
	...
	return (
		<TaskComposerView
			...
			model={{
				...
				onRefresh: selectedAgent === "" ? undefined : () => refreshModels.mutate(),
				onModelChange: (value) => {
					setModel(value);
					setMode("");
	...
	return (
		<span className="inline-flex w-full min-w-0 items-center gap-1.5">
			<input
				...
				placeholder={fetching ? t("settings.models.loading") : t("newTask.autoModel")}
			/>
			{hasCatalog && (
				<AgentModelCombobox
					key={agentId}
